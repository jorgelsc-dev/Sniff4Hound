...
def build_ca(cn: str, years=10):
    ...
    subject = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Sniff4Hound"),
...
def write_env_files(out_dir: Path, master_host: str):
    ...
    (abs_dir / "master.env").write_text(
        "\n".join(
            [
                "export SNIFF4HOUND_ROLE=master",
                "export SNIFF4HOUND_HOST=0.0.0.0",
                "export SNIFF4HOUND_PORT=45678",
                "export SNIFF4HOUND_TLS_ENABLED=1",
                "export SNIFF4HOUND_TLS_REQUIRE_CLIENT_CERT=0",
                f"export SNIFF4HOUND_CA={ca_cert}",
                f"export SNIFF4HOUND_CA_ONELINE='{ca_oneline}'",
                f"export SNIFF4HOUND_TLS_CERT_FILE={master_cert}",
                f"export SNIFF4HOUND_TLS_KEY_FILE={master_key}",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    (abs_dir / "agent.env").write_text(
        "\n".join(
            [
                "export SNIFF4HOUND_ROLE=agent",
                f"export SNIFF4HOUND_MASTER=https://{master_host}:45678",
                f"export SNIFF4HOUND_CA={ca_cert}",
                f"export SNIFF4HOUND_CA_ONELINE='{ca_oneline}'",
                "export SNIFF4HOUND_AGENT_ID=",
                "export SNIFF4HOUND_AGENT_SHARED_KEY=",
                "# Optional mTLS client certificate mode:",
                f"# export SNIFF4HOUND_AGENT_CERT={agent_cert}",
                f"# export SNIFF4HOUND_AGENT_KEY={agent_key}",
                "export SNIFF4HOUND_IP=",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    (abs_dir / "admin.env").write_text(
        "\n".join(
            [
                f"export SNIFF4HOUND_CA={ca_cert}",
                f"export SNIFF4HOUND_ADMIN_CERT={admin_cert}",
                f"export SNIFF4HOUND_ADMIN_KEY={admin_key}",
                "",
            ]
        ),
        encoding="utf-8",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate Sniff4Hound CA, master, admin and agent certificates."
    )
    ...
    parser.add_argument("--ca-cn", default="Sniff4Hound Root CA")
    parser.add_argument("--master-cn", default="sniff4hound-master")
    parser.add_argument("--admin-cn", default="sniff4hound-admin")
    parser.add_argument("--agent-cn", default="sniff4hound-agent")
