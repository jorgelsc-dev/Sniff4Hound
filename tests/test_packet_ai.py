...
class AiStorageTests(unittest.TestCase):
    ...
    def test_clean_traffic_is_never_persisted_regardless_of_training_mode(self):
        ...
        with patch.object(sniffer, '_get_monitor_context', return_value=([], True)), \
            ...
            sniffer._store_packet(packet())
            sniffer._store_packet(packet())
        # Persistence depends only on whether evaluation raised something -
        # Training does not keep a feed of undetected traffic of its own.
        self.assertEqual(self.store.count_packets(), 0)
...
class AiApiTests(unittest.TestCase):
    ...
    def test_ai_alert_mode_requires_raw_packet_retention(self):
        ...
        with patch.object(module, 'store', self.store):
            ...
            self.store.set_raw_retention_enabled(True)
            ...
            # The Dashboard's own toggle for the flag itself.
            result = module.ai_config(request(b'{"raw_retention_enabled":false}'))
            self.assertFalse(result['raw_retention_enabled'])
            self.assertFalse(self.store.get_raw_retention_enabled())
